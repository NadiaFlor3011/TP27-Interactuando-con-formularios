window.onload = function () {
    ...
    const validarCampo = (target, campo, msg, valEsp) => {
        if (target.value === "" || valEsp) {
            ...
            document.getElementById(`small-${campo}`).classList.add("is-invalid")
            document.getElementById(`${campo}`).classList.add("is-invalid")
            document.getElementById(`${campo}`).classList.remove("is-valid")
        } else {
            document.getElementById(`small-${campo}`).innerHTML = ""
            document.getElementById(`small-${campo}`).classList.remove("is-invalid")
    ...
    document.getElementById("formMoviesAdd").addEventListener("submit", (e) => {
        ...
        let errores = []
        for (let x = 0; x < 6; x++) {
            if (e.target[x].value === "" || e.target[x].classList.contains("is-invalid")) {
                if (e.target[x].value === "") {
